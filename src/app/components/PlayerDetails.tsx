...
export function PlayerDetails() {
  ...
  useEffect(() => {
    apiGet<PlayersResponse>("/api/players?limit=200")
      .then((data) => {
        setPlayers(data.players);
        if (data.players.length > 0) setSelectedId(data.players[0].player_id ?? data.players[0].id);
      })
  ...
  return (
    <div className="p-12 space-y-12">
      <div>
        ...
        <p className="text-slate-400 text-xl font-medium">Every dropdown option and stat is loaded from your FastAPI NBA backend.</p>
        {error && (
          <div className="mt-4 flex items-center gap-3 text-red-400 font-semibold">
      ...
      <div className="max-w-xl">
        <label className="block text-sm text-slate-500 font-black uppercase tracking-widest mb-3">Select Live NBA Player</label>
        <select
          value={selectedId ?? ""}
          onChange={(e) => setSelectedId(Number(e.target.value))}
          disabled={loadingPlayers || players.length === 0}
          className="w-full px-6 py-4 bg-slate-950 border-2 border-orange-500/50 rounded-2xl text-white text-lg font-bold outline-none focus:border-orange-400"
        >
          {players.map((player) => (
            <option key={player.player_id ?? player.id} value={player.player_id ?? player.id}>
      ...
      {loadingPlayers || loadingDetails || !profile || !details ? (
        <div className="flex items-center gap-3 text-slate-300 text-xl">
          <Loader2 className="animate-spin" /> Loading live player data...
        </div>
      ) : (
        <>
          <div className="relative p-10 bg-gradient-to-br from-slate-900/90 to-slate-950/90 border-2 border-slate-800 rounded-3xl shadow-2xl overflow-hidden">
            ...
            <div className="relative flex flex-col lg:flex-row lg:items-center gap-10">
              <div className="w-32 h-32 rounded-3xl bg-gradient-to-br from-orange-500 to-red-600 flex items-center justify-center shadow-2xl shadow-orange-500/30">
                ...
                <tbody>
                  {recentGames.map((g, i) => (
                    <tr key={i} className="border-b border-slate-900 hover:bg-slate-900/60">
                      <td className="p-5 text-slate-300">{g.GAME_DATE ?? "--"}</td>
                      <td className="p-5 text-white font-bold">{g.MATCHUP ?? "--"}</td>
                      <td className="p-5 text-orange-400 font-black">{g.PTS ?? "--"}</td>
                      <td className="p-5 text-slate-300">{g.REB ?? "--"}</td>
                      <td className="p-5 text-slate-300">{g.AST ?? "--"}</td>
                      <td className="p-5 text-slate-300">{g.MIN ?? "--"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
